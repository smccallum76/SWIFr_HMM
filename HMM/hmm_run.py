...
if update_db == 'yes':
    ...
    engine = sqlite3.connect(db_path + db_name)
...
---------------------------------------------------------------------------------------------------
"""
state_count = 4  # 2-->[neutral, sweep]; 3-->[neutral, link, sweep]; 4-->[neutral, link_left, sweep, link_right]
cut_point = 0  # set to zero if all data is to be used [>0 crops data to value for shorter run time]
stoch_sims = 2  # number of stochastic simulations to perform
# paths below are to teh example data, but must be revised by the user
gmm_path = '../swifr_pkg/test_data/hmm_example_4class/'
test_path = '../swifr_pkg/test_data/hmm_example_4class_test/test/test'
train_path = '../swifr_pkg/test_data/hmm_example_4class_train/train/train'
...
---------------------------------------------------------------------------------------------------
"""
# define the pi vector using the label_num
...
for stat in stats:
    ...
    delta = hmm.hmm_forward(gmm_params, test[stat], np.copy(A_trans), np.copy(pi), stat=stat)
